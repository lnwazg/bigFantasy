...
public class ServerController extends ServerBaseController
{
     ...
     * 防止并发操作问题
     */
    //    public static List<Connection> ALL_CONNECTIONS = new Vector<>();
    ...
    public static void updateServerConfig()
    {
        ...
        if (serverConfigInfo != null)
        {
            ExecMgr.guiExec.execute(() -> {
                //记录用户上次的浏览位置
                int oldPosition = serverConfigInfo.getCaretPosition();
                //保持原有的滚动条进度
                int oldScrollValue = logScroller.getVerticalScrollBar().getValue();
                //设置新值
                serverConfigInfo.setText(configJson);
                
                if (oldPosition <= configJson.length() - 1)
                {
                    //只有当老位置合法的时候，才回滚到老位置。否则，说明文字内容变短，老位置必然超限，因此采用默认的处理（滚动到最后）
                    serverConfigInfo.setCaretPosition(oldPosition);
                }
                logScroller.getVerticalScrollBar().setValue(oldScrollValue);
            });
        }
    }
